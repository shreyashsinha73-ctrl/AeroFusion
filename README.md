...
- **Multi-Year Historical Archive**: Generates high-resolution static spatial maps (0.05° grid, ~5.5 km per cell) for 2019-2025 covering baseline (Jun-Aug) and biomass burning (Oct-Nov) seasons for both TROPOMI HCHO column density and MODIS/VIIRS Fire Radiative Power (FRP), enabling direct year-over-year comparison of pyrogenic activity and chemical plume intensity over the Indian subcontinent.
...
├── generate_static_maps.py  # Offline batch renderer for 24 historical 
│                              static map PNGs (2019-2025, baseline & 
│                              burning seasons, FRP & HCHO variables)
├── static_maps/             # Pre-rendered PNG archive (generated by 
│                              generate_static_maps.py, read by app.py 
│                              historical mode)
├── models/
...
```bash
pip install numpy pandas xarray scipy tensorflow torch streamlit plotly netcdf4 cartopy geopandas shapely duckdb matplotlib
```
